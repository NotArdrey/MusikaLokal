...
export async function runGigPortfolioReview(client: any, applicationId: string, supabaseUrl: string) {
    ...
    try {
        ...
        // Only files submitted with this application can affect its evidence review.
        // Profile and group portfolio media are intentionally excluded from scoring.
        ...
        const faceSimilarity = !faceComparisonEligible
            ...
            : !profilePhotoUrl
                ...
                : facePlusPlusResults.get('solo-applicant') || unavailableFaceMatch('Face++ face matching did not return a result.', 'The Face++ response was incomplete.')
        const groupFaceSimilarity = groupMemberProfiles.map((member: any) => ({
            profile_id: member.id,
            display_name: cleanText(member.full_name, 120) || 'Group member',
            ...(facePlusPlusResults.get(String(member.id)) || unavailableFaceMatch('No approved group-member profile photo was available.', 'Face++ face matching was not run for this group member.')),
        }))
