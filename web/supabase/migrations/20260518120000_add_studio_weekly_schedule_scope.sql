...
ADD COLUMN IF NOT EXISTS weekly_schedule_scope text DEFAULT 'indefinite' NOT NULL,
ADD COLUMN IF NOT EXISTS weekly_schedule_end_date date,
ADD COLUMN IF NOT EXISTS weekly_schedule_dates jsonb DEFAULT '[]'::jsonb NOT NULL;

ALTER TABLE public.studio_settings
DROP CONSTRAINT IF EXISTS studio_settings_weekly_schedule_scope_check;

ALTER TABLE public.studio_settings
ADD CONSTRAINT studio_settings_weekly_schedule_scope_check
CHECK (weekly_schedule_scope = ANY (ARRAY['indefinite', 'until', 'specific_dates']));

ALTER TABLE public.studio_settings
DROP CONSTRAINT IF EXISTS studio_settings_weekly_schedule_dates_check;

ALTER TABLE public.studio_settings
ADD CONSTRAINT studio_settings_weekly_schedule_dates_check
CHECK (jsonb_typeof(weekly_schedule_dates) = 'array');
...
DECLARE
  ...
  v_weekly_scope text := 'indefinite';
  v_weekly_end_date date;
  v_weekly_dates jsonb := '[]'::jsonb;
BEGIN
  ...
  IF NOT v_has_override THEN
    SELECT
      COALESCE(ss.weekly_schedule_scope, 'indefinite'),
      ss.weekly_schedule_end_date,
      COALESCE(ss.weekly_schedule_dates, '[]'::jsonb)
    INTO v_weekly_scope, v_weekly_end_date, v_weekly_dates
    FROM public.studio_settings ss
    WHERE ss.studio_id = p_studio_id;

    IF v_weekly_scope = 'until'
      AND v_weekly_end_date IS NOT NULL
      AND p_booking_date > v_weekly_end_date THEN
      RETURN FALSE;
    END IF;

    IF v_weekly_scope = 'specific_dates'
      AND NOT (v_weekly_dates ? p_booking_date::text) THEN
      RETURN FALSE;
    END IF;
  END IF;
