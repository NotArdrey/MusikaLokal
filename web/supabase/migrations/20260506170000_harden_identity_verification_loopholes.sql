-- Harden identity verification against duplicate accounts, metadata tampering,
-- webhook replay, direct profile writes, and identity-history loss.
...
begin
  if auth.role() in ('anon', 'authenticated') then
    if tg_op = 'INSERT' then
      ...
      then
        raise exception 'Sensitive verification fields must be written by trusted server code'
          using errcode = '42501';
      end if;
