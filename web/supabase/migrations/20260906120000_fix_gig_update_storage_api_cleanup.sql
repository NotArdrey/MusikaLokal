...
-- Commit gig and media references atomically. Return removed URLs for cleanup
-- through the Storage API after the transaction succeeds.
...
BEGIN
  ...
  -- Storage objects must be removed through the Storage API, never via SQL.
  -- Cleanup failures must not roll back a successfully saved gig.
  v_storage_cleanup := jsonb_build_object(
    ...
    'removed_urls', to_jsonb(v_removed_urls),
