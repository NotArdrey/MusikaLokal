...
-- Treat accepted and approved gig applications as active bookings when gig
-- schedules change or a venue attempts to delete a gig.
...
BEGIN
  ...
  IF v_gig.organizer_id <> v_uid AND NOT public.staff_can_edit_gig(v_uid, p_gig_id) THEN
  ...
  SELECT
    COUNT(*) FILTER (WHERE status IN ('accepted', 'approved')),
    COUNT(*) FILTER (WHERE status = 'pending'),
    COUNT(*) FILTER (WHERE status IN ('accepted', 'approved') AND COALESCE(slot_type, 'solo') = 'solo'),
    COUNT(*) FILTER (WHERE status IN ('accepted', 'approved') AND COALESCE(slot_type, 'solo') = 'duo'),
    COUNT(*) FILTER (WHERE status IN ('accepted', 'approved') AND COALESCE(slot_type, 'solo') = 'band')
  INTO
    v_accepted_total,
    v_pending_count,
    v_accepted_solo,
  ...
  v_major_change := (
    ...
    OR COALESCE(v_new_requirements->'event_schedules', '[]'::jsonb) IS DISTINCT FROM COALESCE(v_existing_requirements->'event_schedules', '[]'::jsonb)
  ...
  SET
    ...
    END
  WHERE id = p_gig_id
    AND (organizer_id = v_uid OR public.staff_can_edit_gig(v_uid, p_gig_id))
  ...
  IF p_payload ? 'requirements' THEN
    ...
    IF jsonb_typeof(v_new_requirements->'event_schedules') = 'array' THEN
      DELETE FROM public.gig_availability_slots
      WHERE gig_id = p_gig_id;

      INSERT INTO public.gig_availability_slots (gig_id, slot_date, start_time, end_time, is_available)
      SELECT
        p_gig_id,
      ...
      FROM (
        SELECT
          (schedule.item->>'date')::date AS slot_date,
          (COALESCE(schedule.item->>'start_time', schedule.item->>'start'))::time AS start_time,
          (COALESCE(schedule.item->>'end_time', schedule.item->>'end'))::time AS end_time,
          schedule.ord
        FROM jsonb_array_elements(v_new_requirements->'event_schedules') WITH ORDINALITY AS schedule(item, ord)
        WHERE NULLIF(btrim(schedule.item->>'date'), '') IS NOT NULL
          AND NULLIF(btrim(COALESCE(schedule.item->>'start_time', schedule.item->>'start')), '') IS NOT NULL
          AND NULLIF(btrim(COALESCE(schedule.item->>'end_time', schedule.item->>'end')), '') IS NOT NULL
      ) AS normalized
      WHERE normalized.end_time > normalized.start_time
  ...
  SELECT COUNT(*) FILTER (WHERE status IN ('accepted', 'approved')),
         COUNT(*) FILTER (WHERE status = 'pending')
...
BEGIN
  ...
  IF v_accepted_count > 0 THEN
    RETURN jsonb_build_object(
      ...
      'message', 'Delete blocked. Resolve accepted or approved applicants first before deleting this gig.'
    );
  END IF;
  ...
  v_applicant_counts := jsonb_build_object(
    ...
    'approved', (SELECT COUNT(*) FROM public.gig_applications WHERE gig_id = p_gig_id AND status = 'approved'),
