...
begin
  ...
  if v_document_fingerprint is null and (v_normalized_name is null or p_birth_date is null) then
    return jsonb_build_object(
      'decision', 'PENDING_REVIEW',
      'claim_id', null,
      'matches', '[]'::jsonb,
  ...
  if v_document_fingerprint is null then
    ...
    where c.user_id = p_user_id
      and c.role = v_role
      and c.document_fingerprint is null
      and c.normalized_full_legal_name = v_normalized_name
      and c.birth_date = p_birth_date
    ...
    if v_claim_id is not null then
      ...
      set normalized_email = nullif(v_email, ''),
          document_type = coalesce(p_document_type, document_type),
          document_type_key = coalesce(p_document_type_key, document_type_key),
          document_country = coalesce(nullif(v_country, ''), 'PHL'),
          verified_full_legal_name = coalesce(v_full_legal_name, verified_full_legal_name),
          normalized_full_legal_name = coalesce(v_normalized_name, normalized_full_legal_name),
          birth_date = coalesce(p_birth_date, birth_date),
          source = coalesce(nullif(p_source, ''), 'DIDIT'),
          status = 'APPROVED',
          didit_session_id = coalesce(p_didit_session_id, didit_session_id),
          manual_review_id = coalesce(p_manual_review_id, manual_review_id),
          claim_metadata = coalesce(claim_metadata, '{}'::jsonb) || coalesce(p_claim_metadata, '{}'::jsonb),
          updated_at = v_now,
          last_seen_at = v_now
      where id = v_claim_id
      returning id into v_claim_id;
  ...
  return jsonb_build_object(
    ...
    'matches', '[]'::jsonb,
    'duplicate_count', 0,
    'matched_on', case when v_document_fingerprint is null then 'NAME_BIRTHDATE' else 'DOCUMENT_FINGERPRINT' end,
