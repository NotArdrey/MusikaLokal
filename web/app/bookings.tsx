...
export default function BookingsScreen() {
  ...
  async function fetchBookings(targetUserId: string) {
    try {
      ...
      const cancelledFromUpcoming = rawUpcoming.filter(
        (item: any) => item.isCancelled || item.status === "Declined" || item.status === "Cancelled"
      );
  ...
  const processClearBalance = async () => {
    ...
    try {
      ...
      debugLog(`Balance cleared: â‚±${balanceAmount} for booking ${bookingId}`);
                      ...
                      <View style={styles.cardHeader}>
                        <View style={styles.cardTitleContainer}>
                          ...
                          <View style={{ marginTop: 8, gap: 4 }}>
                            ...
                            <View style={styles.cardDetailRow}>
                              <Ionicons
                                name={applicationIcon as any}
                                size={14}
                                color={colors.primary}
                              />
                              <Text style={[styles.cardDetailText, { color: colors.textSecondary }]} numberOfLines={2}>
                                {isMusicianView
        ...
        <BlurView intensity={60} tint="dark" style={styles.modalOverlay}>
          ...
          >
            ...
            >
              ...
              >
                Pay half today. Remaining balance: â‚±
                {Math.round(
                  getPaymentItemTotalAmount(paymentItem) / 2,
                ).toLocaleString()}{" "}
                shown in Pending.
