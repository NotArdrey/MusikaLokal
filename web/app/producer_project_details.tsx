...
export default function ProducerProjectDetailsScreen() {
  ...
  const handleApply = async () => {
    ...
    setApplying(true);
    try {
      const { data } = await supabase.functions.invoke("manage-producer-network", { body: { action: "apply_to_project", project_id, role_id: applyRoleId, message: applyMessage.trim() || null } });
            ...
            <View style={styles.section}>
              ...
              {pendingApps.map((app: any) => (
                <View key={app.id} style={[styles.appCard, { backgroundColor: cardBg, borderColor: borderCol }]}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: colors.text, fontWeight: "600" }}>{app.applicant_name}</Text>
      ...
      <Modal visible={showApplyModal} transparent animationType="slide" onRequestClose={() => setShowApplyModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalBox, { backgroundColor: cardBg }]}>
            ...
            <TouchableOpacity style={[styles.submitBtn, { backgroundColor: colors.primary, opacity: applying ? 0.6 : 1 }]} onPress={handleApply} disabled={applying}>
