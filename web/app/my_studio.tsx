...
export default function MyStudioScreen() {
    ...
    }, [fetchDeleteConflicts, handleDeleteSuccess, mapDeleteConflictBooking]);
        ...
        try {
            if (!skipConflictCheck) {
                try {
                    ...
                    console.log('Error preloading delete conflicts:', conflictError);
                }
            }
            ...
            showAlert('error', 'Delete Failed', e instanceof Error ? e.message : 'Failed to delete studio');
        } finally {
            setDeleting(false);
        }
    };
        ...
        try {
            ...
            for (const resolution of resolutions) {
                ...
                if (cancelError || cancelData?.error) {
                    throw cancelError || new Error(cancelData?.error || cancelData?.message || 'Failed to cancel booking.');
                }
            }
            ...
            console.log('Error resolving studio delete conflicts:', e);
            showAlert('error', 'Delete Failed', e instanceof Error ? e.message : 'Failed to resolve bookings before delete.');
        } finally {
            setDeleting(false);
        }
    };
