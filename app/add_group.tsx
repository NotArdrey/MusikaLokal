    ...
    <View className="flex-1" style={{ backgroundColor: colors.background }}>
      ...
      <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
        {step === 1 && (
          <View className="pt-2 pb-24">
            <Text className="text-xl mb-6" style={{ fontFamily: 'Poppins_600SemiBold', color: colors.text }}>Group Details</Text>

            {renderInput('Group Name', groupName, setGroupName, 'e.g. The Jazz Collective')}

            {/* Genre Select Override */}
            <View className="mb-4">
            ...
            {renderInput('Description', description, setDescription, 'Tell us about your group, style, and history...', true)}

            <Text className="mb-2 text-xs uppercase tracking-wider" style={{ fontFamily: 'Poppins_600SemiBold', color: colors.textSecondary }}>Logo / Cover Image</Text>
            <TouchableOpacity
            ...
            >
              ...
              <Text style={{ fontFamily: 'Poppins_500Medium', color: colors.text }}>Upload Image</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {step === 2 && (
          <View className="pt-2 pb-24">
            <Text className="text-xl mb-2" style={{ fontFamily: 'Poppins_600SemiBold', color: colors.text }}>Band Members</Text>
            <Text className="text-sm mb-6" style={{ fontFamily: 'Poppins_400Regular', color: colors.textSecondary }}>Add members to your group profile.</Text>
            ...
            <View className="gap-3">
              {members.map((member, index) => (
                <View key={index} className="flex-row items-center justify-between p-4 rounded-xl border" style={{ backgroundColor: colors.card, borderColor: colors.border }}>
                  <View className="flex-row items-center gap-3">
                    <View className="w-8 h-8 rounded-full bg-indigo-100 items-center justify-center">
                      <Text style={{ fontFamily: 'Poppins_600SemiBold', color: colors.primary }}>{member.charAt(0)}</Text>
                    </View>
              ...
              {members.length === 0 && (
                <View className="p-8 items-center justify-center">
                  <Text style={{ fontFamily: 'Poppins_400Regular', color: colors.textSecondary }}>No members added yet.</Text>
                </View>
              )}
            </View>
          </View>
          ...
          <View className="pt-2 pb-24">
            ...
            <View className="p-5 rounded-2xl border mb-6" style={{ backgroundColor: colors.card, borderColor: colors.border }}>
              <View className="items-center mb-6">
                ...
                <Text className="text-xl" style={{ fontFamily: 'Poppins_700Bold', color: colors.text }}>{groupName || 'Untitled Group'}</Text>
                <Text className="px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-xs mt-2" style={{ fontFamily: 'Poppins_500Medium' }}>{genre || 'No Genre'}</Text>
              </View>

              <View className="border-t pt-4" style={{ borderColor: colors.border }}>
                <Text className="text-xs uppercase text-gray-400 mb-2" style={{ fontFamily: 'Poppins_600SemiBold' }}>Members ({members.length})</Text>
      ...
      <View className="absolute bottom-24 left-0 right-0 px-6 py-4 border-t border-gray-100 bg-white/90" style={{ borderColor: isDark ? '#334155' : '#E5E7EB', backgroundColor: isDark ? colors.background : '#fffc' }}>
        <View className="flex-row gap-4">
          ...
          >
            <Text style={{ fontFamily: 'Poppins_600SemiBold', color: '#fff' }}>{step === 3 ? 'Create Group' : 'Next Step'}</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <Modal
        ...
        title="Create Group"
        message="Are you sure you want to create this group profile?"
