    ...
    return (
        <>
            <View className="flex-1" style={{ backgroundColor: colors.background }}>
                ...
                >
                    {loading ? (
                        <Text style={{ color: colors.textSecondary, textAlign: 'center', marginTop: 20 }}>Loading gigs...</Text>
                    ) : gigs.length === 0 ? (
                        <View className="items-center py-10 opacity-50">
                            <Ionicons name="musical-notes-outline" size={48} color={colors.textSecondary} />
                            <Text className="mt-4" style={{ color: colors.textSecondary }}>No gigs found</Text>
                        </View>
                    ) : (
                        gigs.map((gig) => (
                            <View key={gig.id} className="mb-6 rounded-3xl overflow-hidden" style={{
                                ...
                                <View className="h-48 relative">
                                    ...
                                    <View className="absolute top-4 right-4 bg-white/90 dark:bg-black/60 px-3 py-1 rounded-full">
                                        <Text style={{ fontFamily: 'Poppins_600SemiBold', fontSize: 12, color: colors.primary }}>{gig.status || 'Active'}</Text>
                                    </View>
                                    <View className="absolute bottom-4 left-4 bg-black/60 px-3 py-1.5 rounded-xl">
                                        <Text style={{ fontFamily: 'Poppins_600SemiBold', fontSize: 13, color: '#FFF' }}>₱{gig.budget?.toLocaleString()}</Text>
                                    </View>
                                </View>

                                <View className="p-4">
                                    <Text style={{ fontFamily: 'Poppins_600SemiBold', fontSize: 18, color: colors.text, marginBottom: 2 }}>{gig.name}</Text>
                                    <Text style={{ fontFamily: 'Poppins_500Medium', fontSize: 13, color: colors.primary, marginBottom: 6 }}>
