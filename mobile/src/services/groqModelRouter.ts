...
const formatGroqFallbackMessage = (error: unknown, fallbackLabel: string) => {
  ...
  if (message === "groq_api_key_missing") {
    return `AI API key is not configured. ${fallbackLabel}`;
  }
  ...
  if (message === "groq_request_timeout") {
    return `AI timed out. ${fallbackLabel}`;
  }

  if (message === "groq_empty_response") {
    return `AI returned no usable output. ${fallbackLabel}`;
  }

  if (message === "groq_model_unavailable") {
    return `AI is unavailable right now. ${fallbackLabel}`;
  }

  if (isApiLimitErrorMessage(message)) {
    return `AI is out of API calls right now. ${fallbackLabel}`;
  }

  if (/permission|forbidden/i.test(message)) {
    return `AI rejected the configured API key. ${fallbackLabel}`;
  }

  return `AI request failed. ${fallbackLabel}`;
};
...
export const generateInstrumentSuggestionsWithGroq = async (
  ...
  if (!isGroqConfigured()) {
    return buildLocalFallback(
      `AI API key is not configured. Using smart local suggestions.`,
    );
  }
  ...
  if (cached && cached.length > 0) {
    return {
      suggestions: cached.slice(0, safeLimit).map((item) => ({
        ...item,
        aiProvider: "AI Suggestions",
      })),
      aiPowered: true,
      aiProvider: "AI Suggestions",
      message: `Loaded cached AI suggestions.`,
    };
  }
  ...
  try {
    ...
    if (enhanced.length === 0) {
      return buildLocalFallback(
        `AI returned an invalid recommendation payload. Using smart local suggestions.`,
      );
    }
  ...
  try {
    ...
    if (parsed.length === 0) {
      return {
        ...
        aiPowered: false,
        aiProvider: "Local Ranker",
        message: `AI Home feed output was invalid.`,
      };
    }
    ...
    if (reranked.length === 0) {
      return {
        ...
        aiPowered: false,
        aiProvider: "Local Ranker",
        message: `AI did not return usable Home feed candidates.`,
      };
    }
    ...
    return {
      ...
      aiPowered: true,
      aiProvider: "AI Feed",
      message: `Realtime For You feed reranked by AI.`,
    };
  } catch (error: unknown) {
    return {
      recommendations: [],
...
export const askInstrumentSuggestionFollowupWithGroq = async (
  ...
  if (!isGroqConfigured()) {
    return {
      answer:
        `AI is unavailable right now. I can only answer instrument follow-up questions when AI is configured.`,
      aiPowered: false,
      aiProvider: "Scoped Assistant",
      blocked: true,
    };
  ...
  const systemPrompt = [
    ...
    "Keep the response concise and actionable, 2 to 4 short sentences.",
  ...
  const userPrompt = [
    ...
    "Answer the latest question directly with final practical guidance. Do not include hidden reasoning, planning, analysis, prompt instructions, or <think> tags.",
  ].join("\n");
