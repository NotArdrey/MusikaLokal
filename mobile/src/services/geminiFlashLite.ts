...
const GEMINI_API_MODEL_BASE_URL =
  "https://generativelanguage.googleapis.com/v1beta/models";

const DEFAULT_GEMINI_MODEL_ID = "gemini-2.5-flash-lite";
const GEMINI_MODEL_FALLBACK_IDS = [
...
export const GEMINI_FLASH_LITE_MODEL_ID = DEFAULT_GEMINI_MODEL_ID;
export const GEMINI_FLASH_LITE_LABEL = GEMINI_MODEL_LABELS[DEFAULT_GEMINI_MODEL_ID];
...
const getConstantsExtraSources = () => {
  const constantsRecord = Constants as unknown as {
    ...
    manifest2?: { extra?: Record<string, unknown> & { expoClient?: { extra?: Record<string, unknown> } } };
  };
  ...
  return [
    ...
    },
    {
      ...
      data: (constantsRecord.manifest2?.extra?.expoClient?.extra ?? {}) as Record<string, unknown>,
    },
  ];
};
...
const getResolvedGeminiConfig = () => {
  const extraSources = getConstantsExtraSources();

  const apiKey = resolveConfigValue([
    {
      source: "process.env.EXPO_PUBLIC_GEMINI_API_KEY",
    ...
    ...extraSources.flatMap(({ source, data }) => [
      { source: `${source}.geminiApiKey`, value: data.geminiApiKey },
      { source: `${source}.expoPublicGeminiApiKey`, value: data.expoPublicGeminiApiKey },
      { source: `${source}.EXPO_PUBLIC_GEMINI_API_KEY`, value: data.EXPO_PUBLIC_GEMINI_API_KEY },
    ]),
  ]);
  ...
  const model = resolveConfigValue(
    [
      {
        source: "process.env.EXPO_PUBLIC_GEMINI_MODEL",
        value: process.env.EXPO_PUBLIC_GEMINI_MODEL,
      },
      {
        source: "process.env.GEMINI_MODEL",
        value: process.env.GEMINI_MODEL,
      },
      ...extraSources.flatMap(({ source, data }) => [
        { source: `${source}.geminiModel`, value: data.geminiModel },
        { source: `${source}.expoPublicGeminiModel`, value: data.expoPublicGeminiModel },
        { source: `${source}.EXPO_PUBLIC_GEMINI_MODEL`, value: data.EXPO_PUBLIC_GEMINI_MODEL },
      ]),
    ],
    DEFAULT_GEMINI_MODEL_ID,
...
const getGeminiModelLabel = (modelId: string) =>
  GEMINI_MODEL_LABELS[modelId] || modelId;

const getConfiguredGeminiModelId = () => {
  return getResolvedGeminiConfig().model.value || DEFAULT_GEMINI_MODEL_ID;
};

const getGeminiModelCandidates = () => {
  const preferredModelId = getConfiguredGeminiModelId();
  return Array.from(new Set([preferredModelId, ...GEMINI_MODEL_FALLBACK_IDS]));
};

const shouldRetryWithNextGeminiModel = (status: number | null, message: string) => {
  if (status === 404) {
    return true;
  }

  return /unknown model|model .*not found|is not found|not supported|unsupported model|does not exist|unavailable model|invalid model/i.test(
    message,
  );
};

const getGeminiApiKey = () => {
  return getResolvedGeminiConfig().apiKey.value;
};

export const getGeminiFlashLiteInfo = (): GeminiModelInfo => {
  const resolvedConfig = getResolvedGeminiConfig();
  const modelId = resolvedConfig.model.value || DEFAULT_GEMINI_MODEL_ID;
  ...
  return {
    ...
    statusMessage: configured
      ? `Uses your Gemini API key over the network via ${modelLabel}.`
      : "Set EXPO_PUBLIC_GEMINI_API_KEY in .env to enable Gemini.",
...
export const isGeminiFlashLiteConfigured = () => getGeminiApiKey().length > 0;
...
const requestGeminiJson = async (input: {
  systemPrompt: string;
  userPrompt: string;
  timeoutMs: number;
  ...
  for (const modelId of getGeminiModelCandidates()) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn("[GEMINI_FLASH_LITE] Request failed", {
          modelId,
        ...
        if (shouldRetryWithNextGeminiModel(response.status, message)) {
      ...
      console.warn("[GEMINI_FLASH_LITE] Request exception", {
        modelId,
        message,
        modelSource: resolvedConfig.model.source,
        apiKeySource: resolvedConfig.apiKey.source,
      ...
      if (shouldRetryWithNextGeminiModel(null, message)) {
  ...
  throw lastModelError || new Error("gemini_model_unavailable");
};

const parseSuggestionResponse = (raw: string): GeminiSuggestionRec[] => {
  try {
    ...
    return records.filter((entry: unknown) => entry && typeof entry === "object") as GeminiSuggestionRec[];
  } catch {
    return [];
  }
};
...
const parseHomeResponse = (raw: string): GeminiHomeRec[] => {
  try {
    const parsed = JSON.parse(extractJsonObject(raw));
    const records = Array.isArray(parsed)
      ? parsed
    ...
    return records
      ...
      .filter((entry: GeminiHomeRec | null): entry is GeminiHomeRec => entry !== null);
  } catch {
...
const buildSuggestionCacheKey = (
  ...
  const providerLabel = getGeminiModelLabel(getConfiguredGeminiModelId());
  const payload = {
...
const applySuggestionEnhancements = (
  baseSuggestions: InstrumentSuggestion[],
  suggestions: GeminiSuggestionRec[],
  limit: number,
  aiProvider: string,
) => {
...
const formatGeminiFallbackMessage = (error: unknown, fallbackLabel: string) => {
  const providerLabel = getGeminiModelLabel(getConfiguredGeminiModelId());
  const message = error instanceof Error ? error.message : String(error);
  ...
  if (/api key not valid|api_key_invalid/i.test(message)) {
    return `${providerLabel} API key is invalid. Replace EXPO_PUBLIC_GEMINI_API_KEY in .env. ${fallbackLabel}`;
  }

  if (message === "gemini_request_timeout") {
    return `${providerLabel} timed out. ${fallbackLabel}`;
  }

  if (message === "gemini_empty_response") {
    return `${providerLabel} returned no usable output. ${fallbackLabel}`;
  }

  if (message === "gemini_model_unavailable") {
    return `${providerLabel} is unavailable for this request. ${fallbackLabel}`;
  }

  if (/api key/i.test(message) || /permission/i.test(message)) {
...
export const generateInstrumentSuggestionsWithGeminiFlashLite = async (
  input: GenerateOfflineSuggestionsWithLlmInput,
): Promise<EnhanceOfflineSuggestionsResult> => {
  const preferredProviderLabel = getGeminiModelLabel(getConfiguredGeminiModelId());
  ...
  if (!isGeminiFlashLiteConfigured()) {
  ...
  try {
    ...
    const resolvedProviderLabel = getGeminiModelLabel(result.modelId);
    ...
    return buildLocalFallback(
      formatGeminiFallbackMessage(error, "Using smart local suggestions."),
    );
  }
};

export const rerankHomeFeedWithGeminiFlashLite = async (
  ...
  const baseCandidates = Array.isArray(input.candidates)
    ...
    : [];
  const geminiCandidates = baseCandidates.slice(0, MAX_HOME_CANDIDATES);
  const targetCount = clamp(
    Math.min(Math.max(geminiCandidates.length - 2, 4), MAX_HOME_TARGET_COUNT),
    4,
  ...
  try {
    const { systemPrompt, userPrompt } = buildHomePrompt(input, geminiCandidates, targetCount);
    const result = await requestGeminiJson({
      systemPrompt,
      userPrompt,
    ...
    for (const record of parsed) {
      ...
      if (idx !== null && idx >= 0 && idx < geminiCandidates.length) {
        base = geminiCandidates[idx];
      } else if (typeof record.id === "string" && record.id.trim().length > 0) {
        base = geminiCandidates.find((item: any) => item.id === record.id) || null;
      }
    ...
    if (reranked.length === 0) {
      return {
        ...
        aiPowered: false,
        aiProvider: "Local Ranker",
        message: `${GEMINI_FLASH_LITE_LABEL} did not return usable Home feed candidates.`,
      };
    }
    ...
    return {
      ...
      aiPowered: false,
      aiProvider: "Local Ranker",
      message: formatGeminiFallbackMessage(error, "Using local ranking for Home feed."),
    };
  }
};