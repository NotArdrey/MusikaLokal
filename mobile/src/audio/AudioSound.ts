...
export class AudioSound {
      ...
      if (!player.isLoaded) {
        await new Promise<void>((resolve, reject) => {
          ...
          const listener = player.addListener("playbackStatusUpdate", (status) => {
            if (status.isLoaded || status.error || status.playbackState === "error") {
              clearTimeout(timeout);
              listener.remove();
              if (status.error || status.playbackState === "error") {
                reject(new Error(status.error || "Unable to load this audio."));
              } else resolve();
            }
          });
          if (player.isLoaded) {
  ...
  private readStatus(status: AudioStatus = this.player.currentStatus): PlaybackStatus {
    if (status.error || status.playbackState === "error") {
      return { isLoaded: false, error: status.error || "Audio playback failed." };
    }
