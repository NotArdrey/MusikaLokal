      ...
      for (const resolution of resolutions) {
        if (resolution.action === "cancel") {
          ...
          if (relocationError || relocationData?.success === false) {
            ...
            logActionError("edit_studio.musician_choice_relocation_failed", errorPayload, {
              bookingId: resolution.bookingId,
              studioId,
              userId: conflict.user_id,
            });
            throw new Error(
              `Failed to ask musician to choose a new time: ${formatSupabaseError(errorPayload)}`,
            );
          }
          ...
          if (relocationError || relocationData?.success === false) {
            const errorPayload = relocationError || relocationData?.error;
            logActionError("edit_studio.request_relocation_failed", errorPayload, {
              bookingId: resolution.bookingId,
              studioId,
              userId: movedBookingUserId,
            });
            throw new Error(
              `Failed to request booking relocation: ${formatSupabaseError(errorPayload)}`,
            );
          }
        }
      }
