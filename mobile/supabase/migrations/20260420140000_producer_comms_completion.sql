...
-- Adds contextual chat support for deals and producer projects,
-- atomic slot increment function, and invite expiry support.

-- ══════════════════════════════════════════════════════════════════
-- 1. Add deal_id and producer_project_id columns to conversations
-- ══════════════════════════════════════════════════════════════════
...
CREATE INDEX IF NOT EXISTS idx_conversations_deal ON public.conversations (deal_id) WHERE deal_id IS NOT NULL;
...
--  unless we want deal/project display names embedded. Skip for now.)
